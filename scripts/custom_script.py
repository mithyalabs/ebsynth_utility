...
# The returned values are passed to the run method as parameters.
    ...
    def ui(self, is_img2img):
        with gr.Column(variant='panel'):
            with gr.Column():
                ...
                generation_test = gr.Checkbox(
                    False, label="Generation TEST!!(Ignore Project directory and use the image and mask specified in the main UI)")

            with gr.Accordion("Mask option"):
                mask_mode = gr.Dropdown(choices=["Normal", "Invert", "None", "Don't Override"],
                                        value="Normal", label="Mask Mode(Override img2img Mask mode)")
                inpaint_area = gr.Dropdown(choices=["Whole picture", "Only masked", "Don't Override"],
                                           type="index", value="Only masked", label="Inpaint Area(Override img2img Inpaint area)")
                use_depth = gr.Checkbox(
                    True, label="Use Depth Map If exists in /video_key_depth")
            ...
            with gr.Accordion("ControlNet option"):
                controlnet_weight = gr.Slider(
                    minimum=0.0, maximum=2.0, step=0.01, value=0.5, label="Control Net Weight")
                controlnet_weight_for_face = gr.Slider(
                    minimum=0.0, maximum=2.0, step=0.01, value=0.5, label="Control Net Weight For Face")
                use_preprocess_img = gr.Checkbox(
                    True, label="Use Preprocess image If exists in /controlnet_preprocess")
                gr.HTML(value="<p style='margin-bottom: 0.7em'>\
            ...
            with gr.Accordion("Loopback option"):
                img2img_repeat_count = gr.Slider(
                    minimum=1, maximum=30, step=1, value=1, label="Img2Img Repeat Count (Loop Back)")
                inc_seed = gr.Slider(
                    minimum=0, maximum=9999999, step=1, value=1, label="Add N to seed when repeating ")

            with gr.Accordion("Auto Tagging option"):
                auto_tag_mode = gr.Dropdown(
                    choices=["None", "DeepDanbooru", "CLIP"], value="None", label="Auto Tagging")
                add_tag_to_head = gr.Checkbox(
                    False, label="Add additional prompts to the head")
                add_tag_replace_underscore = gr.Checkbox(
                    False, label="Replace '_' with ' '(Does not affect the function to add tokens using add_token.txt.)")
                gr.HTML(value="<p style='margin-bottom: 0.7em'>\
            ...
            with gr.Accordion("Face Crop option"):
                ...
                with gr.Row():
                    face_detection_method = gr.Dropdown(
                        choices=["YuNet", "Yolov5_anime"], value="YuNet", label="Face Detection Method")
                    gr.HTML(value="<p style='margin-bottom: 0.7em'>\
                ...
                face_crop_resolution = gr.Slider(
                    minimum=128, maximum=2048, step=1, value=512, label="Face Crop Resolution")
                max_crop_size = gr.Slider(
                    minimum=0, maximum=2048, step=1, value=1024, label="Max Crop Size")
                face_denoising_strength = gr.Slider(
                    minimum=0.00, maximum=1.00, step=0.01, value=0.5, label="Face Denoising Strength")
                face_area_magnification = gr.Slider(
                    minimum=1.00, maximum=10.00, step=0.01, value=1.5, label="Face Area Magnification ")
                disable_facecrop_lpbk_last_time = gr.Checkbox(
                    False, label="Disable at the last loopback time")

                with gr.Column():
                    enable_face_prompt = gr.Checkbox(
                        False, label="Enable Face Prompt")
    ...
    def detect_anime_face_from_img(self, img_array):
        ...
        if not self.anime_face_detector:
            ...
            anime_model_path = download_and_cache_models(
                os.path.join(models_path, "yolov5_anime"))
            ...
            self.anime_face_detector = torch.hub.load(
                'ultralytics/yolov5', 'custom', path=anime_model_path)
    ...
    def detect_face(self, img, mask, face_detection_method, max_crop_size):
        ...
        for face in faces:
            ...
            face_coords.append([x/img_array.shape[1], y/img_array.shape[0],
                               w/img_array.shape[1], h/img_array.shape[0]])
    ...
    def get_mask(self):
        def create_mask(output, x_rate, y_rate, k_size):
            ...
            img = cv2.ellipse(img, ((256, 256), (int(
                512 * x_rate), int(512 * y_rate)), 0), (255, 255, 255), thickness=-1)
            img = cv2.GaussianBlur(img, (k_size, k_size), 0)
            cv2.imwrite(output, img)

        if self.face_merge_mask_image is None:
            mask_file_path = os.path.join(
                get_my_dir(), self.face_merge_mask_filename)
    ...
    def face_img_crop(self, img, face_coords, face_area_magnification):
        ...
        for face_img in face_imgs:
            if face_img.shape[1] < face_img.shape[0]:
                ...
                re_h = int(x_ceiling((self.face_crop_resolution /
                           face_img.shape[1]) * face_img.shape[0], 64))
            else:
                re_w = int(x_ceiling((self.face_crop_resolution /
                           face_img.shape[0]) * face_img.shape[1], 64))
    ...
    def face_crop_img2img(self, p, face_coords, face_denoising_strength, face_area_magnification, enable_face_prompt, face_prompt, controlnet_input_img, controlnet_input_face_imgs, preprocess_img_exist):
        ...
        print(face_coords)
        face_imgs, new_coords = self.face_img_crop(
            base_img, face_coords, face_area_magnification)
        ...
        # img2img base img
        proc = self.process_images(
            p, controlnet_input_img, self.controlnet_weight, preprocess_img_exist)
        print(proc.seed)
        ...
        for face, coord, controlnet_input_face in zip(face_imgs, new_coords, controlnet_input_face_imgs):
            ...
            if p.image_mask is not None:
                x, y, w, h = coord
                cropped_face_mask = Image.fromarray(
                    np.array(p.image_mask)[y: y+h, x: x+w])
                face_p.image_mask = modules.images.resize_image(
                    0, cropped_face_mask, face.width, face.height)

            face_proc = self.process_images(
                face_p, controlnet_input_face, self.controlnet_weight_for_face, preprocess_img_exist)
    ...
    def get_depth_map(self, mask, depth_path, img_basename, is_invert_mask):
        ...
        if os.path.isfile(depth_img_path):
            ...
            depth_img_path = os.path.join(
                depth_path, os.path.splitext(img_basename)[0] + "-0000.png")
            if os.path.isfile(depth_img_path):
                depth = Image.open(depth_img_path)
...
#        self.debug_count += 1
    ...
    def interrogate_clip(self, imgs, masks):
        ...
        try:
            ...
            for img, mask, caption in zip(imgs, masks, caption_list):
                ...
                clip_image = shared.interrogator.clip_preprocess(image).unsqueeze(
                    0).type(shared.interrogator.dtype).to(devices.device_interrogate)

                res = ""

                with torch.no_grad(), devices.autocast():
                    image_features = shared.interrogator.clip_model.encode_image(
                        clip_image).type(shared.interrogator.dtype)
                    image_features /= image_features.norm(dim=-1, keepdim=True)

                    for name, topn, items in shared.interrogator.categories():
                        matches = shared.interrogator.rank(
                            image_features, items, top_count=topn)
    ...
    def remove_reserved_token(self, token_list):
        reserved_list = ["pink_background",
                         "simple_background", "pink", "pink_theme"]
    ...
    def add_token(self, token_list):
        ...
        if not os.path.isfile(add_list_path):
            ...
            if self.add_tag_replace_underscore:
                token_list = [(x[0].replace("_", " "), x[1], x[2])
                              for x in token_list]
                ...
                for add_item in add_list:
                    if token[0] == add_item["target"]:
                        if token[1] > add_item["min_score"]:
                            ...
                            result_list.append(
                                (add_item["token"][0], score, add_item["type"]))
...
# what is returned by the process_images method.
    def run(self, p, project_dir, generation_test, mask_mode, inpaint_area, use_depth, img2img_repeat_count, inc_seed, auto_tag_mode, add_tag_to_head, add_tag_replace_underscore, is_facecrop, face_detection_method, face_crop_resolution, max_crop_size, face_denoising_strength, face_area_magnification, enable_face_prompt, face_prompt, controlnet_weight, controlnet_weight_for_face, disable_facecrop_lpbk_last_time, use_preprocess_img):
        ...
        if generation_test:
            ...
            controlnet_input_path = os.path.join(
                test_proj_dir, "controlnet_input")
            ...
            if test_base_img:
                test_base_img.save(os.path.join(
                    test_video_key_path, "00001.png"))
            if test_mask:
        ...
        if mask_mode == "Invert":
            ...
            preprocess_path = os.path.join(
                project_dir, "controlnet_preprocess")

            controlnet_input_path = os.path.join(
                project_dir, "controlnet_input")
        ...
        if not os.path.isdir(controlnet_input_path):
            ...
            print("copy {0} -> {1}".format(org_key_path,
                  controlnet_input_path))
            ...
            for img in imgs:
                ...
                shutil.copy(img, os.path.join(
                    controlnet_input_path, img_basename))
        ...
        for img, mask in zip(imgs, masks):
            ...
            if is_facecrop:
                image = Image.open(img)
                mask_image = Image.open(mask) if mask else None
                face_coords = self.detect_face(
                    image, mask_image, face_detection_method, max_crop_size)
                if face_coords is None or len(face_coords) == 0:
                    print("no face detected")
                else:
                    print("face detected")
        ...
        if not prompts_dict:
            if auto_tag_mode != "None":
                prompts_dict = self.create_prompts_dict(
                    imgs, masks, auto_tag_mode)

                for key, value in prompts_dict.items():
                    prompts_dict[key] = (
                        value + "," + p.prompt) if add_tag_to_head else (p.prompt + "," + value)
        ...
        for img, mask, controlnet_input_img, face_coords, prompts in zip(imgs, masks, controlnet_input_imgs, face_coords_dict.values(), prompts_dict.values()):
            ...
            if mask_mode != "None" or use_depth:
                if use_depth:
                    depth_found, _p.image_mask = self.get_depth_map(
                        mask_image, depth_path, img_basename, is_invert_mask)
                    mask_image = _p.image_mask
                    if depth_found:
                        _p.inpainting_mask_invert = 0

            preprocess_img_exist = False
            controlnet_input_base_img = Image.open(
                controlnet_input_img) if controlnet_input_img else None
            ...
            if face_coords:
                controlnet_input_face_imgs, _ = self.face_img_crop(
                    controlnet_input_base_img, face_coords, face_area_magnification)
            ...
            while repeat_count > 0:
                ...
                if face_coords:
                    proc = self.face_crop_img2img(_p, face_coords, face_denoising_strength, face_area_magnification, enable_face_prompt,
                                                  face_prompt, controlnet_input_base_img, controlnet_input_face_imgs, preprocess_img_exist)
                else:
                    proc = self.process_images(
                        _p, controlnet_input_base_img, self.controlnet_weight, preprocess_img_exist)
                    print(proc.seed)
                ...
                if repeat_count > 0:
                    ...
                    if mask_image is not None and resized_mask is None:
                        resized_mask = resize_img(
                            np.array(mask_image), proc.images[0].width, proc.images[0].height)
